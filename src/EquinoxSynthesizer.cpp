...
EquinoxSynthesizer::EquinoxSynthesizer (StateManager& state, int synthNum) : oscillatorSynth (Synth::voiceType::oscillator), sampleSynth (Synth::voiceType::sampler), stateManager (state), instanceNum (synthNum)
{
...
EquinoxSynthesizer::~EquinoxSynthesizer() = default;
...
void EquinoxSynthesizer::addParameters (std::vector<std::unique_ptr<RangedAudioParameter>>& params)
{
    ...
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("ampAttack" + instanceNumAsString(), 1), "AmpAttack", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("ampDecay" + instanceNumAsString(), 1), "AmpDecay", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 1.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("ampSustain" + instanceNumAsString(), 1), "AmpSustain", NormalisableRange<float> (0.0f, 1.0f, 0.002f, 1.0f), 0.8f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("ampRelease" + instanceNumAsString(), 1), "AmpRelease", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 0.0f));

    // FilterEnvelope parameters
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("filterAttack" + instanceNumAsString(), 1), "FilterAttack", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("filterDecay" + instanceNumAsString(), 1), "FilterDecay", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("filterSustain" + instanceNumAsString(), 1), "FilterSustain", NormalisableRange<float> (0.0f, 1.0f, 0.002f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("filterRelease" + instanceNumAsString(), 1), "FilterRelease", NormalisableRange<float> (0.0f, 10.0f, 0.002f, 0.4f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("envelopeCutoffLimit" + instanceNumAsString(), 1), "EnvelopeCutoffLimit", NormalisableRange<float> (200.0f, 22050.0f, 0.1f, 0.4f), 10000.0f));

    // Waveform parameter
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("waveform" + instanceNumAsString(), 1), "Waveform", 0.0f, 4.0f, 0.0f));

    // isActive parameter - only setting the first EquinoxSynthesizer instance to active
    bool initializeAsActive = instanceNum == 1;
    params.push_back (std::make_unique<AudioParameterBool> (ParameterID ("isActive" + instanceNumAsString(), 1), "On/Off", initializeAsActive));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("amplitude" + instanceNumAsString(), 1), "Amplitude", NormalisableRange<float> (0.0f, 1.0f, 0.001f, 1.0f), 0.7f));

    // General synth parameters
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("detune" + instanceNumAsString(), 1), "Detune", NormalisableRange<float> (-20.0f, 20.0f, 1.0f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("finePitch" + instanceNumAsString(), 1), "FinePitch", NormalisableRange<float> (-100.0f, 100.0f, 1.0f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("oscPanning" + instanceNumAsString(), 1), "OscPanning", NormalisableRange<float> (-1.0f, 1.0f, 0.01f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("pitchTranspose" + instanceNumAsString(), 1), "PitchTranspose", NormalisableRange<float> (-24.0f, 24.0f, 1.0f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("analogValue" + instanceNumAsString(), 1), "AnalogValue", NormalisableRange<float> (1.0f, 40.0f, 1.0f, 1.0f), 1.0f));

    // Filter parameters
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("filterType" + instanceNumAsString(), 1), "FilterType", 0.0f, 3.0f, 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("cutoff" + instanceNumAsString(), 1), "Cutoff", NormalisableRange<float> (5.0f, 22050.0f, 1.0f, 0.3f), 22050.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("resonance" + instanceNumAsString(), 1), "Resonance", NormalisableRange<float> (0.0f, 0.9f, 0.009f, 1.0f), 0.0f));

    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("drive" + instanceNumAsString(), 1), "Drive", NormalisableRange<float> (1.0f, 15.0f, 0.14f, 1.0f), 1.0f));

    // Synth Mode
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("synthMode" + instanceNumAsString(), 1), "SynthMode", 0.0f, 1.0f, 0.0f));

    // Mono & Portamento
    params.push_back (std::make_unique<AudioParameterBool> (ParameterID ("monoEnabled" + instanceNumAsString(), 1), "Mono", false));
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("portamento" + instanceNumAsString(), 1), "Portamento", NormalisableRange<float> (0.0f, 40000.0f, 100.0f, 1.0f), 0.0f));

    // Oscillator Phase
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("phase" + instanceNumAsString(), 1), "Phase", NormalisableRange<float> (0.0f, 1.0f, 0.001f, 1.0f), 0.0f));

    // Sample start time
    params.push_back (std::make_unique<AudioParameterFloat> (ParameterID ("sampleStartTime" + instanceNumAsString(), 1), "Sample start time", NormalisableRange<float> (0.0f, 0.9f, 0.001f, 0.5f), 0.0f));
}
