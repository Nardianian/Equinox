...
{
    float nextSample = 0;
    if (sourceSamplePositionLeft < sampleLength && sourceSamplePositionRight < sampleLength && inL != nullptr && inR != nullptr)
    {
        ...
        *sourceSamplePos += (channel == 0) ? getFrequency() : (getDetune()) ? getFrequency() * (getDetune())
                                                                            : getFrequency();
    }
    return nextSample;
}
